...
local pedModel = GetHashKey("s_m_m_migrant_01")
RequestModel(pedModel)
while not HasModelLoaded(pedModel) do
    Wait(0)
end

local ped = CreatePed(4, pedModel, -1785.03, 2988.78, 31.90, 0.0, false, true)
SetEntityAsMissionEntity(ped, true, true)
SetBlockingOfNonTemporaryEvents(ped, true)
FreezeEntityPosition(ped, true)
SetEntityInvincible(ped, true)

RegisterNetEvent('ta_dialog:openExample', function()
    local dialogData = {
        dialog = {
            {
                id = "greeting",
                name = "Migrant Worker",
                text = "Hello there! Welcome to our humble camp. How can I assist you today?",
                buttons = {
                    {
                        label = "Ask about work",
                        nextDialog = "work_info"
                    },
                    {
                        label = "Say goodbye",
                        close = true
            ...
            },
            {
                id = "work_info",
                name = "Migrant Worker",
                text = "We have some tasks that need doing. Are you interested in helping out?",
                buttons = {
                    {
    ...
    local result = exports.ta_dialog:showDialog(dialogData)
...
exports['qb-target']:AddTargetEntity(ped, {
    options = {
        {
            type = "client",
            event = "ta_dialog:openExample",
            icon = "fas fa-comment",
            label = "Talk to Migrant Worker"
        }
    },
    distance = 2.5
})